...
///
/// The vchat-agent binary (installed via the signup script) generates
...
/// The filename where the vchat-agent stores its Nkey seed (relative to identity dir).
...
pub fn get_public_key(identity_dir: &Path) -> Result<Option<String>> {
    ...
    // Try using the vchat-agent binary to decode the public key
